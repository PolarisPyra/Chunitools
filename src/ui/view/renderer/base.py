...
RenderFunction = Callable[[QPainter, Any, float, Any], None]
...
class RenderCache:
    ...
    def get_pixmap(self, key: Tuple[Any, ...], draw_func: Callable[[QPainter, QRectF], None], width: float, height: float) -> QPixmap:
...
class BaseRenderer(AirRendererMixin, DamageRendererMixin, FlickRendererMixin, HeavenHoldRendererMixin, HoldRendererMixin, SlideRendererMixin):
    """
    ...
    def draw_lane_lines(self, painter: QPainter, current_position: float, top_measure: float, bottom_measure: float) -> None:
        if not hasattr(self, "_lane_pen"):
            self._lane_pen = self.cache.get_pen(QColor(100, 100, 100))
        painter.setPen(self._lane_pen)
        y_start, y_end = self.projection.y(top_measure, current_position), self.projection.y(bottom_measure, current_position)
    ...
    def draw_measure_lines(self, painter: QPainter, start_measure: int, end_measure: int, current_position: float, viewport_width: int, show_labels: bool = True) -> None:
        from src.ui.theme.ui import TEXT_MEASURE, qt
        ...
        for measure_idx in range(start_measure, end_measure + 1):
            ...
            if show_labels:
                painter.save()
                painter.setPen(qt(TEXT_MEASURE))
    ...
    def draw_notes(self, painter: QPainter, notes: List[Any], current_position: float) -> None:
        if not self.projection.timeline_engine: return
        ...
        COMPOSITE_HEAD_TYPES = {
            NoteType.HLD, NoteType.HXD, NoteType.SLD, NoteType.SXD, NoteType.SLC, NoteType.SXC,
            NoteType.ASD, NoteType.ASC, NoteType.ASX, NoteType.AHD, NoteType.AHX, NoteType.ALD
        }
        for note in notes:
            nt_val = note.note_type.value
            if not self.visible_note_types.get(nt_val, True):
                if note.note_type not in COMPOSITE_HEAD_TYPES: continue
    ...
    def _dispatch_note_tasks(self, render_tasks: List[RenderTask], note: Any, timeline: Any) -> None:
    ...
    def _dispatch_air_tasks(self, render_tasks: List[RenderTask], note: Any, note_type: NoteType, tick: int) -> None:
        """Version-aware air task dispatcher."""
        is_legacy = self.major == 1 and self.minor < 13
        
        if note_type in (NoteType.ASD, NoteType.ASC, NoteType.ASX) or isinstance(note, AirSlideStart):
    ...
    def _dispatch_foreground_tasks(self, render_tasks: List[RenderTask], note: Any, note_type: NoteType, tick: int) -> None:
        if note_type in (NoteType.HLD, NoteType.HXD): 
            ...
            render_tasks.append(RenderTask(30, lambda p, n, c, t: self._draw_tap(p, n, c, t, self.colors.tap), note, tick))
        elif note_type == NoteType.CHR: 
            render_tasks.append(RenderTask(30, lambda p, n, c, t: self._draw_tap(p, n, c, t, self.colors.ex_tap), note, tick))
    ...
    def _draw_tap(self, painter: QPainter, note: Any, current_position: float, timeline: Any, color: GradientColor | QColor) -> None:
        if note.note_type == NoteType.SLC:
            if not self.visible_note_types.get(NoteType.SLD.value, True): return
        elif note.note_type == NoteType.SXC:
            if not self.visible_note_types.get(NoteType.SXD.value, True): return
        elif not self.visible_note_types.get(note.note_type.value, True): return
        y, x, w = self.projection.y(timeline.note_abs_pos(note), current_position), self.projection.x(note.cell), self.projection.w(note.width)
        ...
        pixmap = self.cache.get_pixmap(
            pixmap_key, 
            lambda p, r: (self._draw_rounded_rect(p, r, color), self._draw_tap_symbol_for_type(p, r, symbol_type)),
            w, self.constants.HEAD_HEIGHT
        )
    ...
    def _draw_rounded_rect(self, painter: QPainter, rect: QRectF, color: GradientColor | QColor) -> None:
        self._draw_note_base(painter, rect, color); self._draw_border(painter, rect)

    def _draw_note_base(self, painter: QPainter, rect: QRectF, color: GradientColor | QColor) -> None:
        gradient_color = self._gradient_color(color)
        gradient = QLinearGradient(rect.topLeft(), rect.bottomLeft()); gradient.setColorAt(0, gradient_color.light); gradient.setColorAt(1, gradient_color.dark)
        painter.setPen(Qt.PenStyle.NoPen); painter.setBrush(QBrush(gradient)); painter.drawPath(self._note_head_path(rect))

    def _draw_border(self, painter: QPainter, rect: QRectF) -> None:
        border_width = rect.height() * self.constants.BORDER_WIDTH_RATIO
        gradient = QLinearGradient(rect.topLeft(), rect.bottomLeft()); gradient.setColorAt(0, self.colors.border.light); gradient.setColorAt(1, self.colors.border.dark)
        painter.setPen(QPen(QBrush(gradient), border_width)); painter.setBrush(Qt.BrushStyle.NoBrush); painter.drawPath(self._note_head_path(rect))
    ...
    def _draw_tap_symbol(self, painter: QPainter, rect: QRectF) -> None:
        painter.setPen(QPen(Qt.GlobalColor.white, rect.height() * self.constants.BORDER_WIDTH_RATIO))
        y, xs, xe = rect.center().y(), rect.left() + rect.width() * 0.2, rect.right() - rect.width() * 0.2
    ...
    def _color_cache_key(self, color: GradientColor | QColor) -> Any:
        return color.rgba() if isinstance(color, QColor) else (color.light.rgba(), color.dark.rgba())
    ...
    def _gradient_color(self, color: GradientColor | QColor) -> GradientColor:
        if isinstance(color, GradientColor): return color
        return GradientColor(dark=color.darker(130), light=color.lighter(130))

    def _fill_metallic_path(self, painter: QPainter, path: QPainterPath, color_group: GradientColor) -> None:
        rect = path.boundingRect(); gradient = QLinearGradient(rect.topLeft(), rect.topRight()); gradient.setColorAt(0, color_group.dark); gradient.setColorAt(0.3, color_group.light); gradient.setColorAt(0.7, color_group.light); gradient.setColorAt(1, color_group.dark)
        painter.setPen(Qt.PenStyle.NoPen); painter.setBrush(QBrush(gradient)); painter.drawPath(path)
